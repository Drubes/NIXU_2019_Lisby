...
def do_something (opcode, p):
    ...
    if opcode == 13: #'PUSHSY',  HEhehhehe .. PUSSY
        ...
        if sym_ent[value] in sl[ep]:
            temp_var=sl[ep].copy(sym_ent[value])
            ...
            while found == 0:
                if sym_ent[value] in sl[parent]:
                    temp_var = sl[parent].copy(sym_ent[value])
                    found = 1
    ...
    if opcode == 37: #'DECLARE',
        sl[ep][sym_ent[value]]="PLACEHOLDER"
        #sl[value][ep]
    if opcode == 38: #'PRINT',
        val = vs.pop(-1)
        print PB
...
def debug(p):
    try:
        ...
        print S+Y+"environments:"+RS
        for i in range(0,len(sl)):
            print S+Y+str(i).ljust(3)+": "+Z+str(sl[i])+RS
        print S+"call stack:"
        for i in range(0, len(cs)):
            print S+nice_offset(cs[i])
        print PB+RS
    except:
        print "DEBUGER FAIL-- you fail at failing"
'''--------------------------------------------------------------------------'''
